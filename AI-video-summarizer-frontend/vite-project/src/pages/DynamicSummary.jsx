...
const DynamicSummary = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Check if URL exists
      const checkResponse = await fetch(`${API_BASE}/check_url`, {
        method: "POST",
      ...
      if (checkData.exists) {
        // Get dynamic summary
        const summaryResponse = await fetch(`${API_BASE}/dynamic_summary`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            url,
        ...
        if (summaryData.success) {
          ...
          setSummary(`Error: ${summaryData.error}`);
        }
      } else {
        alert(
          "Video not found in database. Please process the video first from the Video Summary or Full Transcription page."
        );
      }
    } catch (error) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = "dynamic_transcription.txt";
    a.click();
      ...
      <div className="dynamic-summary-container">
        <div className="dynamic-content-wrapper">
          ...
          <form onSubmit={handleSubmit} className="dynamic-form">
            ...
            <button type="submit" disabled={loading || !url || !startTime || !endTime}>
              {loading ? "Generating..." : "Get Transcription"}
            </button>
          </form>
          ...
          {loading && (
            <div className="loading">
              ...
              <p>Generating dynamic transcription...</p>
            </div>
          )}
                          ...
                          {languages.map(([code, name]) => (
                            <div
                              ...
                              className={`language-item ${
                                selectedLanguage === code ? "selected" : ""
                              }`}
